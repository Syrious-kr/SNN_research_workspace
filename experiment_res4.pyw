...
def run_ablation_study(modes, res_steps, seeds, epochs=30):
    ...
    for mode in modes:
        ...
        for seed in seeds:
            ...
            optimizer = torch.optim.Adam(model.parameters(), lr=0.003); criterion = nn.CrossEntropyLoss()
...
target_resolution = 4; baselines = ['Ideal_FP32', 'Linear_Quant', 'HfO2_Physics', 'Noise_Aware']
...
plt.title(f'Ablation Study (Hardware Res: {target_resolution} steps, 5 Seeds)'); plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.legend(); plt.grid(True)
plt.savefig('ablation_study_res4.png')
