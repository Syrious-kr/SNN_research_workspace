        ...
        for seed in seeds:
            ...
            for epoch in range(epochs):
                ...
                for img, label in train_loader:
                    ...
                    out_fr = sum(model(img) for _ in range(4)) # T=4로 축소
                    loss = criterion(out_fr / 4, label); loss.backward(); optimizer.step(); functional.reset_net(model)
...
plt.title(f'Ablation Study (Time-Step: 4, Res: {target_resolution}, 5 Seeds)'); plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.legend(); plt.grid(True)
plt.savefig('ablation_study_t4.png')